...
from ..policy.timing_logger import TimingLogger
...
class EmotionEngine:
    def __init__(self) -> None:
        ...
        self._latest_affect_packet: Dict[str, object] = {}
    ...
    def start(self, config: EngineConfig) -> None:
        ...
        if self._llm and self._llm.enabled:
            self._setup_llm_timing_confirm()
    ...
    def _update_risk(
        ...
        self._T = None
        self._last_t_sub = {}
        self._S = self._fusion_scorer.score_with_context(
            self._V, self._A, None,
            personality=self._personality,
            arousal_trend=float((self._latest_affect_packet.get("trend", {}) or {}).get("arousal_delta", 0.0)),
            emotion_stability=float((self._latest_affect_packet.get("trend", {}) or {}).get("emotion_stability", 1.0)),
            trend_label=str((self._latest_affect_packet.get("trend", {}) or {}).get("label", "stable")),
        )
        ...
        if timestamp_ms - self._last_risk_emit_ms >= self._config.runtime.risk_update_interval_ms:
            ...
            # 同步 affect_packet 给后端主动时机判断
            if self._latest_affect_packet:
                emit_payload["affect_packet"] = dict(self._latest_affect_packet)
            self._emit("RiskUpdate", timestamp_ms, emit_payload)
    ...
    def _handle_trigger(self, timestamp_ms: int, decision) -> None:
        ...
        s_final = self._fusion_scorer.score_with_context(
            decision.v, decision.a, t_score,
            personality=self._personality,
            arousal_trend=float((self._latest_affect_packet.get("trend", {}) or {}).get("arousal_delta", 0.0)),
            emotion_stability=float((self._latest_affect_packet.get("trend", {}) or {}).get("emotion_stability", 1.0)),
            trend_label=str((self._latest_affect_packet.get("trend", {}) or {}).get("label", "stable")),
        ...
        if self._care_policy:
            ...
            care_plan = self._care_policy.decide(
                ctx, frame, list(self._history),
                affect_packet=self._latest_affect_packet,
                personality=self._personality,
            )
        ...
        if care_plan and care_plan.decision in {"NUDGE", "CARE", "GUARD"}:
            ...
            if self._personality:
                emit_payload["personality"] = dict(self._personality)
            self._emit("CarePlanReady", timestamp_ms, emit_payload)
        self._append_history(timestamp_ms)
        self._record_summary_event(timestamp_ms, tags, summary, transcript, decision, t_score, s_final)
    ...
    # --- Proactive timing: affect/personality integration ---

    def update_affect_packet(self, packet: Dict[str, object]) -> None:
        """Called by AffectRuntime after each emotion inference cycle.

        Stores the latest affect packet so it can be passed to
        care_policy.decide() for LLM timing confirmation.
        """
        self._latest_affect_packet = dict(packet)

    def update_personality(self, personality: Dict[str, float]) -> None:
        """Called by AssessmentEngine when personality scores are updated.

        These scores (Fe, Fi, etc.) are used by FusionScorer.score_with_context()
        and by the LLM timing confirmation prompt.
        """
        self._personality = {k: float(v) for k, v in personality.items() if isinstance(v, (int, float))}

    def _setup_llm_timing_confirm(self) -> None:
        """Wire up the LLM timing confirmation to CarePolicy."""
        if not self._care_policy or not self._llm:
            return

        def _llm_call(prompt: str) -> str:
            """Simple LLM call for timing judgment.

            Uses the same LLM client as care replies, but with a
            shorter max_tokens and no web search.
            """
            if not self._llm or not self._llm.enabled:
                return ""
            try:
                messages = [
                    {"role": "system", "content": "你是情感关怀时机判断助手。只输出JSON，不要其他内容。"},
                    {"role": "user", "content": prompt},
                ]
                completion = self._llm._create_completion(messages, want_json=True)
                return self._llm._extract_text_from_completion(completion)
            except Exception:
                return ""

        def _timing_confirm(prompt_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
            """Parse LLM timing judgment response."""
            import re as _re
            ...
            try:
                ...
                if isinstance(parsed, dict) and "should_intervene" in parsed:
                    return parsed
            except Exception:
                pass
            ...
            if match:
                try:
                    ...
                    pass
            # Try finding { ... } in text
            match2 = _re.search(r"\{[^{}]*\}", raw, _re.DOTALL)
            if match2:
                try:
        ...
        self._care_policy.set_llm_timing_confirm(_timing_confirm)
