...
TIMING_CHECK_PROMPT = """你是一个情感关怀时机判断助手。根据以下实时信息，判断现在是否应该主动关心用户。

当前情绪状态：
- 情绪标签：{emotion}
- 唤醒度(arousal)：{arousal:.2f}（0=平静，1=极度激动）
- 风险分数(V)：{V:.2f}
- 融合分数(S)：{S:.2f}
- 情绪置信度差：{confidence_gap:.2f}
- 唤醒度变化趋势：{arousal_delta:+.3f}
- 情绪稳定性：{emotion_stability:.2f}
- 趋势标签：{trend_label}
...
规则引擎建议：{rule_decision}（分数={rule_score:.2f}）
...
请回复JSON，只包含：
{
  ...
  "confidence": 0.0到1.0,
  "reason": "简短理由，不超过20字",
  "suggested_level": "nudge或care或guard"
}

判断原则：
1. 唤醒度持续上升且情绪负面 => 更倾向干预
2. I型(Fi>Fe)人格 => 降低干预意愿
3. 用户情绪不稳定(频繁切换) => 倾向干预但用nudge而非care
4. 规则分数<0.5 => 不干预
5. 规则分数>=0.9且有负面情绪 => 应干预为guard
6. 如果不确定，宁可nudge而非care，宁可不干预而非过度干预"""


class CarePolicy:
    def __init__(self, templates_path: str) -> None:
        self.templates = self._load_templates(Path(templates_path))
        self._llm_confirm_fn: Optional[Callable[[Dict[str, Any]], Optional[Dict[str, Any]]]] = None

    def set_llm_timing_confirm(self, fn: Callable[[Dict[str, Any]], Optional[Dict[str, Any]]]) -> None:
        """Register an LLM timing confirmation function.

        The function receives a dict with emotion/arousal/personality/rule_decision
        and should return a dict with:
          - should_intervene: bool
          - confidence: float
          - reason: str
          - suggested_level: str ("nudge"/"care"/"guard")
        If it returns None or should_intervene=False, the original rule decision is kept.
        """
        self._llm_confirm_fn = fn

    def decide(
        self,
        ctx: Context,
        frame: RiskFrame,
        history: List[RiskFrame],
        *,
        ...
        # Determine initial rule-based decision
        rule_decision = "RECORD_ONLY"
        if S >= thresholds.get("guard", 0.9):
            rule_decision = "guard"
        elif S >= thresholds.get("care", 0.7):
            rule_decision = "care"
        elif S >= thresholds.get("nudge", 0.6):
            rule_decision = "nudge"

        # LLM timing confirmation: when rules say intervene, ask LLM to confirm
        if rule_decision != "RECORD_ONLY" and self._llm_confirm_fn is not None:
            llm_result = self._llm_confirm_timing(
                frame, S, rule_decision, affect_packet, personality
            )
            if llm_result is not None:
                should = bool(llm_result.get("should_intervene", True))
                llm_level = str(llm_result.get("suggested_level", rule_decision)).strip().lower()
                llm_reason = str(llm_result.get("reason", "")).strip()
                llm_confidence = float(llm_result.get("confidence", 0.5))
                if not should:
                    # LLM says don't intervene — downgrade to RECORD_ONLY
                    return CarePlan(
                        text="",
                        style="warm",
                        motion={},
                        ...
                        policy={"interrupt": False, "timing_source": "llm", "timing_action": "cancelled"},
                        decision="RECORD_ONLY",
                        level=0,
                        steps=[],
                        cooldown_min=cooldown_min,
                        record_event=True,
                        event_type="LLM_TIMING_CANCEL",
                    )
                # LLM confirms — use LLM suggested level (may upgrade or downgrade)
                if llm_level == "guard" and S >= thresholds.get("guard", 0.9):
                    return build_guard_plan(
                        ctx, frame,
                        {"pattern": "llm_confirmed_guard", "S": S, "llm_reason": llm_reason, "llm_confidence": llm_confidence},
                        cooldown_min,
                    )
                if llm_level == "care" and S >= thresholds.get("nudge", 0.6):
                    return build_care_plan(
                        ctx, frame,
                        {"pattern": "llm_confirmed_care", "S": S, "llm_reason": llm_reason, "llm_confidence": llm_confidence},
                        cooldown_min,
                    )
                return build_nudge_plan(
                    ctx, frame,
                    {"pattern": "llm_confirmed_nudge", "S": S, "llm_reason": llm_reason, "llm_confidence": llm_confidence},
                    cooldown_min,
                )

        # Original rule-based path (no LLM confirmation available)
        if S >= thresholds.get("guard", 0.9):
    ...
    def _llm_confirm_timing(
        ...
        """Ask the LLM to confirm or cancel the intervention timing.

        Returns None if LLM is not available or returns unparseable result.
        Returns a dict with should_intervene/confidence/reason/suggested_level otherwise.
        """
        if self._llm_confirm_fn is None:
            return None
        pkt = affect_packet or {}
        trend = pkt.get("trend", {}) or {}
        try:
            prompt_data = {
                "emotion": pkt.get("emotion", "unknown"),
                "arousal": float(pkt.get("arousal", frame.A)),
                "V": float(frame.V),
                "S": float(S),
                "confidence_gap": float(pkt.get("confidence_gap", 0.0)),
                "arousal_delta": float(trend.get("arousal_delta", 0.0)),
                "emotion_stability": float(trend.get("emotion_stability", 1.0)),
                "trend_label": str(trend.get("label", "stable")),
                "Fe": float((personality or {}).get("Fe", 0.0)),
                "Fi": float((personality or {}).get("Fi", 0.0)),
                "rule_decision": rule_decision,
                "rule_score": float(S),
            }
            prompt_text = TIMING_CHECK_PROMPT.format(**prompt_data)
            result = self._llm_confirm_fn({"timing_check_prompt": prompt_text, **prompt_data})
            if result is None:
                return None
...
def build_nudge_plan(ctx: Context, fr: RiskFrame, reason: Dict[str, Any], cooldown_min: int) -> CarePlan:
    ...
    return CarePlan(
        ...
        followup_question="",
        reason=reason,
        policy={"interrupt": True, "content_source": "template"},
        decision="NUDGE",
        level=1,
        steps=steps,
        cooldown_min=cooldown_min,
