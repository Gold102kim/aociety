...
async function run() {
  ...
  try {
    ...
    assert.deepEqual(completed.basicQuestionnaire.interests, ['游戏', '音乐', '旅行']);
    ...
    assert.deepEqual(completed.aiAgent.preferences.interests, ['游戏', '音乐', '旅行']);
