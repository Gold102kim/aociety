...
NPCS = (
    (
        "AvatarSample_A",
        r"E:\Aociety-NEW\SourceAssets\Characters\NPC_VRoid\Export\AvatarSample_A\SK_AvatarSample_A.fbx",
    ),
    (
        "AvatarSample_C",
        r"E:\Aociety-NEW\SourceAssets\Characters\NPC_VRoid\Export\AvatarSample_C\SK_AvatarSample_C.fbx",
    ),
)
ROOT_DESTINATION = "/Game/Aociety/Characters/NPC"
...
for name, fbx_path in NPCS:
    if not os.path.isfile(fbx_path):
        raise RuntimeError(f"Missing NPC FBX: {fbx_path}")
...
for task in tasks:
    ...
    total_imported += len(imported_paths)
    for path in imported_paths:
        print(f"[AocietyNPCImport] {path}")

unreal.EditorAssetLibrary.save_directory(
    ROOT_DESTINATION, only_if_is_dirty=False, recursive=True
)
print(f"[AocietyNPCImport] characters={len(tasks)} assets={total_imported}")
